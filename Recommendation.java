...
public class Recommendation extends javax.swing.JFrame {
    ...
    public Recommendation() {
        ...
        try
        {
            ...
            String gamerating = ("SELECT * FROM GameRatings");
            // Displaying the library(later use)
            ResultSet dispLib = st.executeQuery("SELECT * FROM GAMES WHERE Game_ID ='"+gamerating+"'");
            for(int i  = 1; i < dispLib.getRow(); i++)
            {
                dispLib.getRowId(i);
                String id = dispLib.getString("Rating_ID");
                String game_id = dispLib.getString("Game_ID");
                String user_id = dispLib.getString("User_ID");
                int rating = dispLib.getInt("Rating");
                newImg = new ImageIcon(getClass().getResource(id));
                for(int x = 1; x < 35; x++)
                {
